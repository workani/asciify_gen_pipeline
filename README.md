...
For a resumable run of that same fixed set, use `node bin/generator.mjs run --100 --stage=records --limit=100 --threads=4 --verbose`. It keeps checkpoints in the main `state.sqlite` and updates `out/run100.json` after each result. Its terminal output matches the live harness, including the final JSON; `--verbose` keeps this compact presentation for the fixed cohort. Both entry points end each character result line with `(x/100)`. On resume, the count includes saved passed, failed and quarantined results; retrying a character does not count it twice. Run the same command again to resume.
